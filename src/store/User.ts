...
const instance = createStore({
  ...
  getters: {
    ...
    isBusiness(state, getters) {
      return getters.isAuthenticated && state.user?.user_type?.toLowerCase() === 'business'
    },
    ...
    hasBusinessProfile(state, getters) {
      return getters.isBusiness && state.user?.business_profile !== null
    },
    cartItemCount(state) {
  ...
  mutations: {
    setUser(state, user: any) {
      ...
      if (!user || !user.user_type) {
        console.warn('user_type non défini ou invalide dans setUser')
        return
      }
      ...
      if (userType !== 'business') {
        state.user.business_profile = null
      }
    ...
    addItemToCart(state: State, data) {
      const existingItem = state.cart.find((c) => c.item.id === data.item.id)
      ...
      localStorage.setItem('cartItemCount', state.cart.length.toString())
    },
  ...
  actions: {
    async googleLoginRedirect({ commit }) {
      ...
      await axios
        .get('https://vmi2584358.contaboserver.net/api/users/social/o/google-oauth2/', {
          params: { redirect_uri },
    ...
    async loginGoogle({ commit }, { state, code }) {
      await axios
        .post(
          'https://vmi2584358.contaboserver.net/api/users/social/o/google-oauth2/',
          { state, code },
          {
    ...
    async login({ commit }, { email, password }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/users/token/login/', { email, password }).then((response) => {
        commit('setToken', response.data.token)
        commit('setExpiry', response.data.expiry)
        commit('setUser', response.data.user)
    ...
    async login2({ commit }, { email_or_phone }) {
      try {
        const response = await axios.post(
          'https://vmi2584358.contaboserver.net/api/otp/login/',
          { email_or_phone }
        )
    ...
    async logout({ commit }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/users/token/logout/').then(() => {
        commit('clearState')
        commit('clearCart')
      })
    },
    async forgotPassword({ commit }, { email }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/users/reset_password/', { email })
    },
    async resetPassword({ commit }, { uid, token, new_password }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/users/reset_password_confirm/', { uid, token, new_password })
    },
    async activateUser({ commit }, { uid, token }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/users/activation/', { uid, token }).then((response) => {
        commit('clearState')
        commit('setToken', response.data.token)
        commit('setExpiry', response.data.expiry)
        commit('setUser', response.data.user)
    ...
    async registerUser({ commit }, { name, email, contact_number, password, user_type }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/users/', { name, email, contact_number, password, user_type })
    },
    async sendOtp({ commit }, { phone }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/otp/send/', { phone })
      return {
        success: true,
      }
    },
    async verify({ commit }, { phone, otp }) {
      await axios.post('https://vmi2584358.contaboserver.net/api/otp/verify/', { phone, otp })
    },
    async verify_otp({ commit }, { phone, otp }) {
      try {
        await axios.post('https://vmi2584358.contaboserver.net/api/otp/verify-otp-token/', { phone, otp })
          .then((response) => {
            const token = response.data.token
    ...
    async registerBusinessProfile({ commit }, { cr_number }) {
      try {
        const response = await axios.post('https://vmi2584358.contaboserver.net/api/users/business-profile/', {
          cr_number
        })
    ...
    async patchBusinessProfile(
      ...
      try {
        ...
        const response = await axios.patch('https://vmi2584358.contaboserver.net/api/users/business-profile/', formData, {
          headers: {
    ...
    async patchUser({ commit }, { user }) {
      await axios.patch('https://vmi2584358.contaboserver.net/api/users/me/', user).then((response) => {
    ...
    async deleteAccount({ commit }, { current_password }) {
      await axios.delete('https://vmi2584358.contaboserver.net/api/users/me/', { data: { current_password } }).then(() => {
        commit('clearState')
      })
    },
    async getUser({ commit }) {
      await axios.get('https://vmi2584358.contaboserver.net/api/users/me/').then((response) => {
        commit('setUser', response.data)
      })
    },
...
// Export pour l'utilisation avec Composition API
