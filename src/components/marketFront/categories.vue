...
export default defineComponent({
    ...
    categoryGroups: {
      ...
      handler(newVal) {
        if (newVal?.length) {
          ...
          // Optionnel : Mettre à jour selectedCategoryId si utile
          this.selectedCategoryId = newVal[0]?.id ?? null
        }
      }
    }
  },
  ...
  methods: {
    onCategoryClick(categoryGroup: CategoryGroup) {
      ...
      this.selectedCategoryId = categoryGroup.id; // Mise à jour ID sélectionné
      console.log('categoryGroup:', categoryGroup) // correction typo console.log
    }
  },
  setup() {
