...
import type { Fleet, ValidationsStructure } from '@/models/Item'
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      additionalServiceFees: 0, // Ajoutez cette ligne
      loadingFees: false,  
      stepTitles: [
  ...
  methods: {
    ...
    async fetchCertificates() {
      try {
        const response = await this.$axios.get('https://vmi2584358.contaboserver.net/api/inventory/certificates/')
    ...
    async createListing(payload: RentalListing) {
      const response = await this.$axios.post('https://vmi2584358.contaboserver.net/api/inventory/user/items/', [payload])
    ...
    async uploadSubItems(savedSubItemsValues: any[]) {
      try {
        ...
        await this.$axios.post(`https://vmi2584358.contaboserver.net/api/inventory/user/items/item-files/`, formData, {
    ...
    async updateListing(payload: RentalListing) {
      ...
      const response = await this.$axios.patch(
        `https://vmi2584358.contaboserver.net/api/inventory/user/items/${this.item.id}/`,
        changedData
      )
  ...
  try {
    ...
    // ✅ Vérifie si le checkbox des frais de service est coché ici
  ...
  created() {
    this.initializeData(), this.fetchCertificates(),this.fetchServiceFees() 
  }
})
</script>
<template>
