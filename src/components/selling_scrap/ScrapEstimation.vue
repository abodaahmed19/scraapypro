...
const fetchCategories = async () => {
  const res = await fetch('https://vmi2584358.contaboserver.net/api/inventory/all-category-alt/')
  const data = await res.json()

  const allGroups = data.message || []
  return allGroups.flatMap(group => group.categories || [])
}
...
const calculateEstimatedAmount = async () => {
 ...
 for (const item of selectedItems) {
  const category = allCategories.find(cat =>
    ...
    cat.name_ar?.toLowerCase() === item.name?.toLowerCase()
  )

    if (category) {
      const price = parseFloat(category.price2) || 0
...
<template>
  <div class="estimation-container">
    ...
    <div class="amount-container">
      <div class="amount">{{ estimatedAmount.toLocaleString() }} <component class="SAR" :is="sarIcon" /></div>
    </div>
