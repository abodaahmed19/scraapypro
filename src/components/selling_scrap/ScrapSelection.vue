...
const fetchScrapTypes = async () => {
  try {
    ...
    scrapTypes.value = json.message?.filter((g: any) => g.group_type === 'product') ?? [];
  ...
  <div class="scrap-selection">
    ...
    <div v-if="!selectedCategoryName" class="selection-content">
      <div class="step-header">
        ...
        <h2 class="step-title" v-if="t('auth.lang') == 'en'">{{ t('selling_scrap.choose') }} <span class="accent">{{ t('selling_scrap.scrap') }}</span> {{ t('selling_scrap.type') }}</h2>
        <h2 class="step-title" v-if="t('auth.lang') == 'ar'">{{ t('selling_scrap.choose') }} {{ t('selling_scrap.type') }} <span class="accent">{{ t('selling_scrap.scrap') }}</span></h2>
      </div>
