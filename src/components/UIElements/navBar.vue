...
export default defineComponent({
  ...
  computed: {
    ...
    marketPlaceFilters() {
      ...
      filters.push({
          label: this.$t('burger.service_request'),
          value: 'service_request',
          route: { name: 'ServiceRequest', query: { tab: 'service_request' } }
        })
      return filters
    }
  },
  ...
  <div :class="['nav-container', { mobile: isMobile, 'menu-open': isMenuOpen, 'scroll-active': isScrolled }]">
    <div class="nav-logo">
      ...
      <div v-if="!isMobile" class="links">
        ...
        <router-link :to="{ name: 'ServiceRequest', query: { tab: 'service_request' } }"  class="link" :class="{ active: activeTab === 'service_request' }">
          {{ $t('burger.service_request') }}
        </router-link>
      </div>
    </div>
    <div class="nav-content">
      ...
      <div class="cart-btn-wrapper" @click="$router.push({ name: 'checkout' })">
        ...
        <span class="num-cart" v-if="$userStore.state.cart.length !== 0">{{ $userStore.state.cart.length }}</span>
      </div>
