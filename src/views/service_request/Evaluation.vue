<template>
  ...
  <div v-if="showSuccess" :class="[$t('auth.lang') === 'ar' ? 'toast-left' : 'toast-right']" class="success-toast">
    <i :class="[$t('auth.lang') === 'ar' ? 'icon-left' : 'icon-right']" class="fas fa-check-circle"></i>
    {{ $t('service_request.doneSend') }}
  </div>
</transition>
...
<script>
  ...
  category_group: 50
        ...
        const response = await axios.post(
          ...
          payload,
          {
            headers: {
              ...
              // Si besoin : ajout du token d'authentification
        ...
        alert('Erreur lors de l\'envoi : ' + (error.response?.data?.detail || 'Erreur inconnue'));
      } finally {
...
/* Positionnement dynamique pour RTL/LTR */
.toast-right {
  right: 20px;
}
