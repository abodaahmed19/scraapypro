...
async function fetchCategoriesForGroup(groupName) {
  try {
    const response = await fetch('https://vmi2584358.contaboserver.net/api/inventory/all-category-alt/')
    const json = await response.json()
    console.log('Réponse API:', json)

    if (!json.message || !Array.isArray(json.message)) {
      throw new Error("Structure de la réponse inattendue : 'message' absent ou non tableau")
    }

    // Filtrer les groupes qui ont group_type = 'product'
    const productGroups = json.message.filter(group => group.group_type === 'product')

    // Trouver le groupe correspondant au nom (insensible à la casse)
...
function submitNewItem() {
  ...
  // البحث عن عنصر بنفس الاسم
  const existingItem = scrapItems.value.find(item => item.name === newItem.value.name)
      ...
      <div class="scrap-selection">
        ...
        <div class="selection-content">
          <section class="card">
            ...
            <button v-if="t('auth.lang') == 'en'" @click="handlePrevious" class="rev-btn">{{ t('selling_scrap.choose') }} {{ t('selling_scrap.new') }} {{ t('selling_scrap.type') }}</button>
            <button v-if="t('auth.lang') == 'ar'" @click="handlePrevious" class="rev-btn">{{ t('selling_scrap.choose') }} {{ t('selling_scrap.type') }} {{ t('selling_scrap.new') }}</button>
          ...
          <div class="modal">
            ...
            <form @submit.prevent="submitNewItem">
              <select v-model="newItem.name" required>
                ...
                <option v-for="category in categoriesFromGroup" :key="category.id" :value="t('auth.lang') == 'en' ? category.name : category.name_ar">
                  {{ t('auth.lang') == 'en' ? category.name : category.name_ar }}
                </option>
              </select>

              <input v-model.number="newItem.quantity" type="number" :placeholder="t('selling_scrap.quantity')" min="0.01" step="0.01" required />
