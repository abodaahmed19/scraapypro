    ...
    <div v-else class="vendors">
      ...
      >
        <div class="icon">
          <img
            v-if="!vendor.image"
            src="@/assets/svg-icons/vendor-placeholder.svg"
        ...
        <div class="content">
          ...
          <div class="details">{{ vendors.length }} {{ $t('management.listingRequests') }}</div>
        </div>
        <div class="actions">
...
export default defineComponent({
  ...
  methods: {
    async fetchApi() {
      ...
      try {
        const { data } = await this.$axios.get(`https://vmi2584358.contaboserver.net/api/inventory/staff/?type=${type}`)
    ...
    async fetchCategories() {
      try {
        const response = await this.$axios.get('/api/inventory/categories/?type=service')
