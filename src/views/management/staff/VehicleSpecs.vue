<template>
  <div class="product_requests" v-auto-animate>
    ...
    <!-- vehicle-specs Inputs when a vehicle-specs is selected -->
    ...
    <div v-else class="categories">
      ...
      <div
        v-for="specs in categories"
        :key="specs.category_group.id"
        class="item"
        @click="openCategory(specs)"
      >
        <div class="icon">
          <img
            v-if="!specs.category_group.icon"
            src="@/assets/svg-icons/vendor-placeholder.svg?url"
            alt="icon"
          />
          <img v-else :src="specs.category_group.icon" alt="icon" />
        </div>
        <div class="content">
          <div class="name">
            {{ $i18n.locale === 'ar' ? specs.category_group.name_ar : specs.category_group.name }}
          </div>
          <div class="details">
...
export default defineComponent({
  ...
  methods: {
    async fetchApi() {
      ...
      try {
        const { data } = await this.$axios.get('https://vmi2584358.contaboserver.net/api/inventory/staff/vehicle-specs/pending/')
    ...
    async handleApproveRequest() {
      ...
      try {
        const response = await this.$axios.post(
          `https://vmi2584358.contaboserver.net/api/inventory/staff/vehicle-specs/approve/${this.items[0].id}/`,
          { ...this.items[0] }
        )
    ...
    async handleRejectItem(reason: string) {
      ...
      try {
        await this.$axios.post(`https://vmi2584358.contaboserver.net/api/inventory/staff/vehicle-specs/reject/${this.rejectItem.id}/`, {
          staff_note: reason
        })
