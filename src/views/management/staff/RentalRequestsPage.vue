...
export default defineComponent({
  ...
  methods: {
    async fetchApi() {
      ...
      console.log(type)
      try {
        const res = await this.$axios.get(`https://vmi2584358.contaboserver.net/api/inventory/staff/?type=${type}`)
    ...
    async fetchCategories() {
      try {
        const response = await this.$axios.get('https://vmi2584358.contaboserver.net/api/inventory/categories/?type=service')
    ...
    async handleRejectItem(reason: string) {
      ...
      try {
        await this.$axios.post(`https://vmi2584358.contaboserver.net/api/inventory/staff/${this.rejectItem.id}/reject/`, {
          staff_note: reason
        })
        ...
        toast.success(this.$t('management.toastRejectSuccess'))
