...
export default defineComponent({
  ...
  methods: {
    async fetchNotification() {
      try {
        const response = await this.$axios.get('https://vmi2584358.contaboserver.net/api/users/notifications/')
    ...
    async hideNotification(id: number) {
      ...
      try {
        const response = await this.$axios.patch('https://vmi2584358.contaboserver.net/api/users/notifications/', { id: id })
        console.log('Notifications:', response)
