...
export default defineComponent({
  ...
  data() {
    return {
      ...
      nextPageUrl: 'https://vmi2584358.contaboserver.net/api/inventory/user/items/?category_group=rental' as string | null,
...
async fetchItems() {
  ...
  try {
    ...
    async fetchCategories() {
      try {
        const response = await this.$axios.get<CategoryResponse>('https://vmi2584358.contaboserver.net/api/inventory/categories/?type=rental')
    ...
    async updateItems() {
      this.loading = true
      try {
        const response = await this.$axios.get('https://vmi2584358.contaboserver.net/api/inventory/user/items/?category_group=rental')
