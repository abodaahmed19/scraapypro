  ...
  methods: {
    ...
    async fetchOrders() {
      ...
      try {
        ...
        const fullUrl = `https://vmi2584358.contaboserver.net/api/billing/orders/?${new URLSearchParams(params).toString()}`
        console.log('Request URL:', fullUrl)

        const response = await this.$axios.get('https://vmi2584358.contaboserver.net/api/billing/orders/', { params })
...
<template>
  ...
  <div v-else class="rentedOut">
    ...
    <div v-if="isLoading" class="loading">
      {{ $t('errorsMsgs.loading') }}
    </div>
    <div v-if="error" class="error">
      {{ error }}
      <MainBtn @click="retryFetch">{{ $t('errorsMsgs.retry') }}</MainBtn>
    </div>
  </div>
</template>
