...
export default defineComponent({
  ...
  emits: ['save', 'close', 'assem', 'update:modelValue'],
  ...
  data() {
    return {
      ...
      priceUnitOptions: [
        ...
        { label: 'طن', value: 'ton' },
        { label: 'لتر', value: 'l' },
        { label: 'عدد/حبة', value: 'pc' },
      ],
      NameOptions: [
        { label: 'شحن', value: 'شحن' },
        { label: 'مناولة', value: 'مناولة' },
        { label: 'فحص', value: 'فحص' },
        { label: 'مختبر', value: 'مختبر' },
        { label: 'تثمين وتقيم', value: 'تثمين وتقيم' },
        { label: 'خدمات هدم', value: 'خدمات هدم' },
        { label: 'حاويات هدم وترميم', value: 'حاويات هدم وترميم' },
        { label: 'حاويات تجارية', value: 'حاويات تجارية' },
        { label: 'مكبس نفايات', value: 'مكبس نفايات' },
        { label: 'نقل بالاجرة', value: 'نقل بالاجرة' },
      ],
    }
  },
  validations() {
...
handleItemData() {
  ...
  // Initialize price_unit properly
  if (!clean_data.price_unit) {
    clean_data.price_unit = this.item.price_unit || 'kg' // Use existing or default
  }
  ...
  return clean_data
    ...
    try {
      ...
      if (response.data && response.data.message.length > 0) {
        ...
        this.additionalServiceFees = parseFloat(firstFee.service_fees_seller) || 0;
        
        console.log('Service fees seller:', this.additionalServiceFees);
      } else {
        console.warn('Aucun frais trouvé');
        this.additionalServiceFees = 0;
      }
    } catch (error) {
    ...
    async checkRequirments(id: any) {
      try {
        const response = await this.$axios.post(`https://vmi2584358.contaboserver.net/api/users/business-profile/missingdata/`, {
          id: [id]
        })
    ...
    async uploadImage(id?: number) {
      ...
      for (const file of files) {
        ...
        await this.$axios
          .post('https://vmi2584358.contaboserver.net/api/inventory/user/items/' + item_id + '/images/', formData, {
            headers: {
    ...
    async setThumbnail(imageID: number) {
      ...
      await this.$axios
        .patch('https://vmi2584358.contaboserver.net/api/inventory/user/items/' + this.item.id + '/images/' + imageID + '/', {
          is_thumbnail: true
        })
        .then((res) => {
          this.itemData.images.forEach((image) => {
    ...
    async deleteImage(imageID: number) {
      ...
      await this.$axios
        .delete('https://vmi2584358.contaboserver.net/api/inventory/user/items/' + this.item.id + '/images/' + imageID + '/')
        .then(() => {
...
async addListing() {
  ...
  // Only include price_unit if category is "other"
  if (this.otherCategory && this.itemData.price_unit) {
    data['price_unit'] = this.itemData.price_unit
  } else {
    // For non-other categories, use the category's default price_unit
    data['price_unit'] = this.selectedCategory?.price_unit || 'kg'
  }
  ...
  await this.$axios
    .post('https://vmi2584358.contaboserver.net/api/inventory/user/items/', [data])
    .then((response) => {
    ...
    async saveChanges() {
      ...
      await this.$axios
        .patch('https://vmi2584358.contaboserver.net/api/inventory/user/items/' + this.item.id + '/', data)
        .then((response) => {
  ...
  try {
    ...
    // ✅ Vérification du checkbox des frais de service (ajoute ici)
...
<div class="service-fee-checkbox">
  ...
  <label for="acceptServiceFee" class="service-fee-label">
    ...
    <span class="service-fee-amount">{{ additionalServiceFees }}</span> 
      ...
      <div class="btn-group" v-if="!disabled">
        <div class="btn-item">
          <MainBtn type="green" :disabled="!hasUnsavedChanges" @click="handleSave">
            <div class="btn-content">
              {{ itemData.id ? $t('listings.saveChanges') : $t('order.save') }}
            </div>
          </MainBtn>
        </div>
        <div class="btn-item">
          ...
          >
            <div class="btn-content">{{ $t('listings.addNew') }}</div>
          </MainBtn>
        </div>
      </div>
    </div>
...
.service-fee-checkbox input[type="checkbox"] {
  margin: 12px;
  width: 20px;
  height: 20px;
}
