...
export default defineComponent({
  ...
  data() {
    ...
    return {
      ...
      businessIcon: localStorage.getItem('sidebarImageUrl') || null // 
  ...
  userImage(): string | null {
    ...
    return `https://https://vmi2584358.contaboserver.net/${img}`
  },
...
async uploadImage(file: File) {
  ...
  formData.append('image', file)  // <-- ici on envoie "image", pas "icon"

  try {
    const response = await this.$axios.patch('https://vmi2584358.contaboserver.net/api/users/me/', formData, {
...
if (updatedImage && typeof updatedImage === 'string') {
  ...
  this.businessIcon = fullImageUrl // <-- ici on met à jour la donnée
