    ...
    <div class="invoice_services">
      ...
      <div class="services_body">
        <div class="service_row" v-for="(item, index) in billing.items" :key="index">
          ...
          <div>15%</div>
          <div>
            {{ (Number(item.price ?? 0) * Number(item.order_quantity ?? 0) * 1.15).toFixed(2) }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="totals">
      <div class="total_container">
        ...
        <p>
          ...
          <span>{{ billing.total_discount ?? 0 }}</span>
        </p>
        <p v-if="billing.tax_amount != null">
          <strong>{{ $t('invoice.Tax') }} (15%)</strong>
          {{ Number(billing.tax_amount ?? 0).toFixed(2) }}
        </p>
        <p>
          <strong>{{ $t('invoice.total') + ' (' + $t('invoice.SAR') + ')' }}</strong>
          <span class="grand_total">{{ Number(billing.total_with_tax ?? 0).toFixed(2) }}</span>
        </p>
      </div>
    </div>
...
export default defineComponent({
  ...
  data() {
    ...
    return {
      store,
      companyInfo: null as Invoice | null,
  ...
  mounted() {
    this.companyInfo = this.store.state.user
  }
})
</script>
